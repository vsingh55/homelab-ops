...
> **Target Baseline:** [`current vs future.md`](file:///home/vsc/devlopment/myGH/homelab-ops/current%20vs%20future.md) & [`process/gitops_implementation_standards.md`](file:///home/vsc/devlopment/myGH/homelab-ops/process/gitops_implementation_standards.md)  
...
7. [Comprehensive Architecture Decision Records](#7-comprehensive-architecture-decision-records)
   ...
   - [ADR-011: Zero-Cost Cloud Extension: OCI Always Free Mumbai for Out-of-Band Monitoring (Uptime Kuma) and Offsite Restic Backup Sync](#adr-011-zero-cost-cloud-extension-oci-always-free-mumbai-for-out-of-band-monitoring-and-offsite-restic-backup-sync)
...
8. [Architectural Contradictions & Resolution Register](#8-architectural-contradictions--resolution-register)
9. [ADR Implementation Roadmap](#9-adr-implementation-roadmap)
10. [Top 5 CTO Executive Recommendations](#10-top-5-cto-executive-recommendations)
11. [Questions / Missing Information Required From User](#11-questions--missing-information-required-from-user)
...
1. **Zero Recurring Infrastructure Cost:** Eliminating ongoing cloud subscriptions in favor of owned on-premises hardware supplemented by validated Always Free cloud tiers.
...
- **Out-of-Band Cloud Extension:** Oracle Cloud Infrastructure (OCI) Always Free tenancy in Mumbai (`ap-mumbai-1`) hosting Uptime Kuma external health probes, **Terraform remote S3 state backend**, and an offsite encrypted Restic backup target.
- **Application Fleet:** 
  - *Public Web:* Preiya's Coaching Portfolio (`preiya.vijaysingh.cloud`), Homelab Architecture Docs (`docs.vijaysingh.cloud`).
...
- Oracle Cloud Free Tier stability in `ap-mumbai-1`.
...
3. **Public Exposure vs. Zero Trust Access Boundary:** Public domains (`preiya.*`, `docs.*`, `hooks.*`) permit anonymous edge traffic; private management interfaces (`dash.*`, `books.*`, `docs-ocr.*`) are gated by Cloudflare Access MFA (Google SSO / Email OTP).
...
| Dimension | Current Baseline State | Future Planned State (Modernization Blueprint) |
| :--- | :--- | :--- |
...
| **Cloud Infrastructure Cost** | ~$7.00–$12.00/month in GCP VM compute, static IP reservation, and cross-region egress. | **₹0.00 / month ($0.00)** via OCI Always Free Tier + Cloudflare Free Tier. GCP fully decommissioned. |
| **Out-of-Band Monitoring** | Internal-only Kube-Prometheus-Stack. ISP drops result in silent blackouts. | OCI Mumbai Always Free VM running Uptime Kuma probing Cloudflare endpoints and Tailscale gateway. |
| **Public Web Applications** | None hosted locally (`vijaysingh.cloud` hosted externally on Hostinger). | Preiya's Coaching Portfolio (`preiya.vijaysingh.cloud`) and Homelab Docs (`docs.vijaysingh.cloud`) on K3s. |
...
| **Disaster Recovery Tier** | Proxmox vzdump + Velero to local MinIO + Restic to local HDD. Zero offsite copy. | Complete 3-2-1 DR: NVMe (Hot) -> HDD (Local Cold) -> OCI Mumbai Always Free Object Storage (Offsite). |
...
6. **Cloudflare Zero Trust Tunnels for Edge Ingress (Proposed - Modernization):** Replacing the paid cloud gateway VM and WireGuard tunnel with outbound `cloudflared` daemons to eliminate costs, cut latency, and hide home IP.
7. **Pull-Based GitOps Continuous Delivery via Flux CD v2 (Proposed - Modernization):** Adopting Flux CD v2 with trunk-based directory overlays, health checks, dependency sequencing, and automated image updates.
8. **Mozilla SOPS + Age Encryption for In-Git Kubernetes Secrets (Proposed - Modernization):** Standardizing on CNCF-compliant in-repo secret encryption, decrypted dynamically inside K3s by Flux CD.
9. **Hardened Decoupling of the n8n Automation Engine (Proposed - Modernization):** Eliminating `hostNetwork: true`, stripping root privileges, revoking node SSH keys from pods, and implementing an event-driven webhook remediation pattern.
10. **CloudNativePG (CNPG) High-Availability Database Architecture (Proposed - Modernization):** Replacing single-pod PostgreSQL with a declarative CNPG operator managing HA replication and continuous WAL archiving to MinIO.
11. **Zero-Cost Offsite Cloud Extension via OCI Always Free (Proposed - Modernization):** Establishing an OCI Mumbai Ampere A1 instance for out-of-band Uptime Kuma monitoring and offsite encrypted Restic backup synchronization.
12. **Hybrid Identity & Access Architecture (Proposed - Modernization):** Using Cloudflare Access with Google SSO / Email OTP for browser-based private web apps, and Tailscale for underlying node administration.
13. **Cold Storage Tiering for Sovereign Media & Documents (Recommended - Future):** Enforcing strict volume isolation and dedicated PVC mount patterns on the 1TB HDD for Paperless-ngx, BookOrbit, and Audiobookshelf.
14. **Deferral of Canary Deployments for Single-Replica Workloads (Recommended - Future):** Postponing complex Flagger canary configurations until workloads scale beyond single-pod instances.
15. **Decommissioning of ops-center Virtual Machine (Accepted - Modernization):** Eliminating the 2GB RAM KVM VM in favor of remote Terraform state in OCI Always Free Object Storage and direct laptop administration over Tailscale, reclaiming 2GB RAM, 2 vCPUs, 20GB NVMe, and 250GB HDD.
...
4. **GitOps Engine Standardization (Contradiction):** Documentation contained a conflict between ArgoCD (promoted in `README.md` and Phase 3 roadmaps) and Flux CD v2 (formalized in `gitops_implementation_standards.md`).
...
- **Recommended Action:** Provision an encrypted offsite backup target in Oracle Cloud Infrastructure Always Free Object Storage in Mumbai, synchronized nightly via Restic over an encrypted channel.
...
- **Observation:** `gitops_implementation_standards.md` mandates Flagger canary deployments with 10% traffic stepping for Preiya's portfolio website and homelab applications.
...
| **P0** | **ADR-011: Zero-Cost Cloud Extension via OCI Always Free for DR & Monitoring** | Establishes the missing offsite tier for true 3-2-1 disaster recovery and enables true out-of-band outage alerting. |
...
- **Scalability Considerations:** Free tier supports up to 100 devices and 3 users, far exceeding homelab requirements.
...
- **Decision:** We will **decommission the GCP Compute Gateway entirely** and deploy **Cloudflare Zero Trust Tunnels (`cloudflared`)** directly inside the `k3s-prod` cluster:
  ...
  3. Ingress routes will map public hostnames directly through `cloudflared` to the internal Traefik ingress service:
     - Public Portfolio: `preiya.vijaysingh.cloud` -> Traefik -> `website-preiya:80`
...
- **Alternatives Considered:**
  ...
  - *Migrate WireGuard Gateway to OCI Always Free VM in Mumbai:* Solves cost and latency, but still requires maintaining a cloud VM, Nginx proxy, Let's Encrypt certificates, and manual WireGuard keepalive watchdogs.
...
- **Consequences:**
  - *Positive:* Latency drops from ~500ms to <15ms; cloud bill reduced by 100% ($0/mo); zero open ports on firewall; automated edge SSL/TLS; automatic static asset CDN caching for Preiya's portfolio and documentation sites.
...
- **Migration Plan:**
  ...
  2. *Phase 2:* Configure CNAME records in Cloudflare DNS for `preiya`, `docs`, and `hooks` pointing to the tunnel ID.
...
- **Validation / Fitness Functions:**
  - TTFB for `preiya.vijaysingh.cloud` must measure <50ms from Indian test vantage points.
...
- **Review Conditions:** Cloudflare introduces tunnel egress bandwidth caps or alters free tier policies.
...
- **Decision:** We will standardize on **Flux CD v2** implementing a **Trunk-Based Development model with Directory Overlays**:
  ...
  2. *Repository Structure:*
     ...
     - `kubernetes/apps/`: Individual application Kustomize overlays (`n8n`, `website-preiya`, `bookorbit`, etc.).
...
```
================================================================================
ADR-011: Zero-Cost Cloud Extension: OCI Always Free Mumbai for Out-of-Band Monitoring (Uptime Kuma) and Offsite Restic Backup Sync
================================================================================
```
- **Status:** Proposed (Active Modernization Blueprint)
- **Date:** September 2026
...
- **Decision:** We will provision an **Oracle Cloud Infrastructure (OCI) Always Free instance in the Mumbai region (`ap-mumbai-1`)**:
  1. *Compute Instance:* Provision an Always Free VM (Ampere A1 ARM shape or E2.1.Micro) managed via Terraform (`infrastructure/oci/`).
  2. *FinOps Safeguard:* Enforce a strict ₹1.00 budget threshold and email alarm in OCI to guarantee zero unexpected billing.
  3. *Out-of-Band Monitoring (Uptime Kuma):* Deploy Uptime Kuma on the OCI VM to probe public endpoints (`preiya.vijaysingh.cloud`, `docs.vijaysingh.cloud`, `hooks.vijaysingh.cloud`) and home router connectivity via Tailscale, sending instant alerts to Slack `#homelab-alerts`.
  4. *Offsite 3-2-1 Backup Target:* Configure an encrypted Restic repository backed by OCI Always Free Object Storage (or block storage on the VM), synchronizing database dumps, configurations, and document assets nightly via AES-256 encryption.
  5. *Terraform S3 Remote State Backend:* Host the remote Terraform state bucket in OCI Always Free Object Storage in Mumbai, enabling resilient offsite state locking and decoupling state survival from local hardware availability (per ADR-015).
...
- **Alternatives Considered:**
  - *AWS Free Tier:* Rejected because AWS EC2 Free Tier expires after 12 months, leading to unexpected billing traps.
...
- **Decision Rationale:** OCI provides the industry's most generous permanent Always Free tier (4 ARM OCPUs, 24GB RAM, 200GB block storage, 10TB egress) located directly in Mumbai, perfectly fulfilling our out-of-band requirements at ₹0.00 cost.
- **Consequences:**
...
- **Review Conditions:** OCI alters Always Free policies or reclaims idle compute shapes.
...
- **Consequences:**
  ...
  - *Negative:* Third-party dependency on Cloudflare Access free tier (up to 50 users).
...
- **Scalability Considerations:** Free tier covers 50 users, far exceeding the projected 3-year requirement of 25 users.
...
- **Cost Considerations:** Infrastructure: ₹0.00 (Cloudflare Zero Trust Free Tier).
- **Migration Plan:**
...
- **Context:** Section 10 of `gitops_implementation_standards.md` details the implementation of **Flagger** for automated Canary / Blue-Green releases with metric-driven rollback (analyzing request success rates and incrementally shifting traffic 10% -> 50% -> 100%).
- **Problem Statement:** In a single-node homelab running on 16GB RAM where applications (Preiya's portfolio, documentation, n8n) run as single-replica deployments, does implementing Flagger progressive delivery justify the significant architectural complexity and resource overhead?
- **Decision Drivers:**
...
- **Decision:** We will **formally defer Flagger progressive canary delivery** for the current and 1-year roadmap:
  ...
  2. *Static Websites:* Preiya's portfolio and documentation sites will rely on Cloudflare Edge CDN caching and instant atomic container replacement.
...
- **Decision:** We will **completely decommission and delete the `ops-center` Virtual Machine**:
  1. *Terraform State Migration:* Migrate the remote Terraform state backend from local MinIO to **Oracle Cloud Infrastructure (OCI) Always Free Object Storage in Mumbai (`ap-mumbai-1`)**. OCI provides 20 GB of S3-compatible, encrypted, versioned object storage with state locking at ₹0.00 cost, completely decoupled from local hardware survival.
...
- **Decision Rationale:** Moving Terraform state to OCI Always Free Object Storage achieves true offsite disaster resilience, saves 2GB of physical host RAM, eliminates proxy latency, and significantly reduces operational complexity.
- **Consequences:**
...
- **Scalability Considerations:** 20GB Always Free storage exceeds Terraform state requirements (<10MB) by over 2,000x.
...
- **Cost Considerations:** Infrastructure: ₹0.00 (OCI Always Free Tier).
- **Migration Plan:**
...
- **Dependencies:** OCI Always Free account in `ap-mumbai-1`, Tailscale client on laptop and Proxmox.
- **Open Questions:** None.
...
- **Scalability Considerations:** Frees sufficient RAM for all 9 planned production applications and 2 websites.
...
## 8. Architectural Contradictions & Resolution Register

During our architectural analysis of the legacy documents (`README.md`, `process/01-existing-infrastructure-overview.md`, `process/02-gcp-migration-guide.md`) against the modernized blueprint (`current vs future.md`, `process/gitops_implementation_standards.md`), five significant architectural contradictions were uncovered:

### Contradiction 1: Ingress Edge Architecture & Cloud Provider Choice
- **The Conflict:** `README.md` and `process/01-existing-infrastructure-overview.md` establish a paid GCP Compute Engine gateway in Mumbai (`asia-south1`) running WireGuard and Nginx, while `current vs future.md` documents a GCP VM running in South Carolina (`us-east1`) costing $7–$12/month, and the future plan introduces Cloudflare Tunnels with an OCI Mumbai Always Free VM.
- **Classification:** **Accidental Evolution:** The GCP deployment began as an experiment in Mumbai, was recreated in `us-east1`, suffered from latency and recurring billing, and was subsequently targeted for replacement.
- **Resolution:** **Resolved via ADR-006 & ADR-011.** Completely decommission GCP resources (`infrastructure/gcp/`). Standardize public ingress exclusively on Cloudflare Zero Trust Tunnels, and establish OCI Always Free in Mumbai for monitoring and backup replication.

### Contradiction 2: GitOps Controller Engine: ArgoCD vs. Flux CD v2
- **The Conflict:** `README.md` (Phase 3 Roadmap, Line 65) explicitly specifies: *"Migrating from Ansible-push to ArgoCD. The cluster will sync itself with this repo"*. Conversely, `current vs future.md` and `process/gitops_implementation_standards.md` mandate **Flux CD v2** based on Linux Foundation LFS269 standards.
- **Classification:** **Intentional Refinement:** ArgoCD was the initial conceptual choice, but deeper analysis revealed that Flux CD v2 is dramatically lighter on cluster memory (~150MB vs ~800MB), integrates seamlessly with Mozilla SOPS, and is better suited for resource-constrained K3s single-node hardware.
- **Resolution:** **Resolved via ADR-007.** Formally deprecate the ArgoCD proposal and standardize 100% on Flux CD v2.

### Contradiction 3: Identity & Access Management: Self-Hosted Keycloak vs. Cloudflare Access
- **The Conflict:** `README.md` (Line 66) lists *"Single Sign-On (SSO): Centralizing access for service, Traefik, and SSH under one identity provider with Keycloak and OIDC"*. In contrast, `current vs future.md` specifies a hybrid model using **Cloudflare Zero Trust Access** (Google SSO / Email OTP) for web apps and **Tailscale** for SSH/Proxmox.
- **Classification:** **Intentional Refinement:** Running a full Java-based Keycloak instance on a 16GB Mini PC hosting 9 other workloads consumes >1GB of critical RAM and introduces massive database and certificate complexity.
- **Resolution:** **Resolved via ADR-012.** Formally reject self-hosted Keycloak. Enforce edge SSO via Cloudflare Access at 0MB host memory cost, and retain Tailscale for administrative node SSH.

### Contradiction 4: Secrets Management: Ansible Vault Hydration vs. Mozilla SOPS + Age
- **The Conflict:** `docs/12-secret-management-hydration.md` establishes Ansible Vault with local Jinja2 hydration generating `terraform.tfvars` on disk as the project standard. However, `gitops_implementation_standards.md` mandates **Mozilla SOPS + Age** encryption for in-Git secrets.
- **Classification:** **Intentional Domain Separation:** Ansible Vault was designed for bare-metal host bootstrapping and Terraform provisioning, but cannot be parsed natively by pull-based in-cluster GitOps controllers.
- **Resolution:** **Resolved via ADR-004 & ADR-008.** Bounded Context Separation: Ansible Vault hydration is retained *strictly* for Day-0 bare-metal host bootstrapping (`infrastructure/on-prem/`). All in-cluster Kubernetes application secrets are standardized on Mozilla SOPS + Age (`kubernetes/`).

### Contradiction 5: Compute Offloading: Serverless GCP Cloud Run vs. Sovereign Local Compute
- **The Conflict:** `README.md` (Line 62) proposes an *"Event-Driven Hybrid Pattern: MinIO upload events trigger Cloud Run containers in GCP for heavy OCR tasks"*. In contrast, `current vs future.md` and `process/sovereign-cloud/Project-2-Implementation-Guide.md` implement a sovereign, locally compiled Alpine Paperless-ngx OCR worker running directly on the homelab 1TB HDD tier.
- **Classification:** **Intentional Strategic Pivot:** Offloading documents to GCP Cloud Run violates data sovereignty principles, introduces egress/ingress networking complexity, and creates unnecessary cloud vendor dependencies.
- **Resolution:** **Resolved via ADR-013.** Retain all document OCR and media processing locally on the 1TB HDD storage tier, scheduled at low CPU priority, maintaining 100% data sovereignty.

---

## 9. ADR Implementation Roadmap

```mermaid
gantt
    title Homelab Modernization & ADR Implementation Roadmap
    dateFormat  YYYY-MM-DD
    section Immediate (Days 1-7)
    ADR-009: Workload Hardening (Purge hostNetwork & SSH) :crit, imm1, 2026-09-10, 3d
    ADR-006: Cloudflare Tunnels Deployment & Latency Cut   :crit, imm2, 2026-09-12, 4d
    ADR-008: SOPS + Age Secret Setup & Repository Keys     :imm3, 2026-09-14, 3d
    section Next 30 Days
    ADR-007: Flux CD v2 Bootstrap & Directory Restructure  :next1, 2026-09-17, 7d
    ADR-010: CloudNativePG HA Operator & WAL Archiving     :next2, 2026-09-24, 6d
    ADR-011: OCI Mumbai Setup & True 3-2-1 Offsite DR      :next3, 2026-10-01, 7d
    ADR-012: Cloudflare Access Google SSO Configuration    :next4, 2026-10-08, 5d
    section Next 90 Days
    ADR-013: Sovereign Media Tiering on 1TB HDD            :mid1, 2026-10-15, 10d
    Deploy Public Websites (Preiya Portfolio & Docs)       :mid2, 2026-10-25, 7d
    Decommission Legacy Standalone Postgres & GCP VM       :mid3, 2026-11-01, 5d
    Slack Multi-Channel ChatOps Integration                :mid4, 2026-11-06, 5d
    section Ongoing / Monitor
    Monitor Host RAM Headroom (<85% threshold)             :mon1, 2026-11-15, 60d
    Monitor OCI Always Free Resource Status                :mon2, 2026-11-15, 60d
    ADR-014: Evaluate Flagger Canary when Scaling >3 Pods  :mon3, 2026-12-01, 30d
```

### Immediate (Days 1–7): Critical Security & Cost Elimination
1. **ADR-009 (Immediate Security Fix):** Strip `hostNetwork: true` from `apps/n8n/n8n.yaml`, remove root privileges from initContainers, delete `remediate.py`, and revoke host SSH keys from the container.
2. **ADR-006 (Public Ingress & Latency):** Deploy `cloudflared` in K3s; map `preiya`, `docs`, and `hooks` to Cloudflare Anycast edge; verify sub-300ms UI loads.
3. **ADR-008 (Secret Foundation):** Generate Master Age keypair; configure `.sops.yaml`; load private key into K3s `sops-age` secret.

### Next 30 Days: Platform Automation & Data Integrity
1. **ADR-007 (GitOps Engine):** Execute `flux bootstrap github`; establish protected `main` branch; deploy platform Kustomizations.
2. **ADR-010 (Database Resilience):** Deploy CloudNativePG operator; provision PostgreSQL cluster with MinIO continuous WAL streaming; migrate n8n database off standalone pod.
3. **ADR-011 (Offsite Disaster Recovery & Monitoring):** Provision OCI Always Free VM in Mumbai via Terraform; establish ₹1 budget alarm; deploy Uptime Kuma for out-of-band Slack alerting; configure nightly encrypted Restic push to OCI.
4. **ADR-012 (Zero Trust Identity):** Configure Cloudflare Access policies with Google SSO for private web dashboards (`dash`, `books`, `docs-ocr`).

### Next 90 Days: Application Expansion & Workload Onboarding
1. **ADR-013 (Sovereign Media Tier):** Format and partition `/mnt/hdd` data directories; deploy BookOrbit (multi-user reading), Audiobookshelf, and Paperless-ngx with dedicated HDD volume claims.
2. **Public Web Launch:** Onboard Preiya's Coaching Portfolio (`preiya.vijaysingh.cloud`) and Homelab Documentation (`docs.vijaysingh.cloud` via Astro + Starlight).
3. **Infrastructure Cleanup:** Execute `terraform destroy` on `infrastructure/gcp/` to permanently stop all GCP billing.
4. **ChatOps Centralization:** Configure Flux and Prometheus alerting to dedicated Slack channels (`#daily-briefing`, `#documents`, `#homelab-alerts`, `#infra-monitoring`, `#deployments`).

### Monitor & Threshold Reconsideration
1. **Cluster RAM Utilization:** Monitor total host memory via Prometheus. If utilization persistently exceeds 85% (>13.6GB RAM), trigger physical RAM upgrade to 32GB or prune non-essential lab containers.
2. **OCI Always Free Tenure:** Monitor OCI console monthly to verify compute instances remain labeled Always Free and within free tier limits.
3. **ADR-014 (Canary Trigger):** Re-evaluate Flagger progressive delivery only if application workloads scale beyond 3 replicas and daily requests exceed 100,000.

---

## 10. Top 5 CTO Executive Recommendations

As Chief Technology Officer evaluating this architecture for long-term viability, maintainability, and enterprise excellence, these are the top 5 strategic imperatives:

### Priority 1: Neutralize the Hypervisor Attack Vector in n8n (Security & Integrity)
- **The Problem:** The internet-accessible n8n container operates with `hostNetwork: true`, `child_process` enabled, and mounts private SSH keys allowing automated execution of root shell commands on the physical Proxmox hypervisor.
- **Business Impact:** A security breach compromises not just homelab data, but the operator's entire home personal network, personal documents, and family assets.
- **Technical Impact:** Breaches container isolation, invalidates Kubernetes network policies, and violates basic zero-trust standards.
- **Urgency:** **IMMEDIATE (Day 1 / P0)**
- **Recommended Action:** Strip `hostNetwork`, drop root permissions, delete the SSH remediation ConfigMap, and restrict n8n to standard container networking with strict egress policies.

### Priority 2: Establish True Offsite 3-2-1 Disaster Recovery (Resilience & Business Continuity)
- **The Problem:** All existing backup layers (vzdump, Velero, Restic) write to the same physical SATA hard drive inside the single Mini PC chassis.
- **Business Impact:** A hardware failure, power surge, theft, or fire results in catastrophic, 100% irrecoverable loss of all sister's portfolio assets, historical documents, and automation workflows.
- **Technical Impact:** False sense of security; system lacks true disaster recovery.
- **Urgency:** **CRITICAL (Week 1 / P0)**
- **Recommended Action:** Provision OCI Always Free Object Storage in Mumbai and implement automated nightly encrypted Restic backups to establish an offsite DR copy at ₹0.00 cost.

### Priority 3: Modernize Ingress to Cloudflare Tunnels and Decommission GCP (FinOps & UX)
- **The Problem:** Ingress routing through a GCP VM in South Carolina introduces ~500ms latency, causes 20-second SPA canvas load times, and incurs ~$7–$12/month in recurring fees.
- **Business Impact:** Degraded user experience for visitors to Preiya's portfolio; ongoing unnecessary cloud expense that violates project financial objectives.
- **Technical Impact:** Complex double-proxy networking with WireGuard MTU fragmentation risks.
- **Urgency:** **HIGH (Week 2 / P0)**
- **Recommended Action:** Deploy `cloudflared` in K3s, route traffic through India edge PoPs (<15ms latency), and run `terraform destroy` on `infrastructure/gcp/` to achieve permanent ₹0.00 monthly spend.

### Priority 4: Enforce Declarative GitOps via Flux CD v2 & SOPS (Operational Velocity)
- **The Problem:** Cluster management relies on manual `kubectl apply` commands and push-based Ansible playbooks, leading to configuration drift and unversioned cluster state.
- **Business Impact:** Rebuilding the cluster after a disaster requires hours of manual troubleshooting; lack of auditable deployment history.
- **Technical Impact:** Drift between Git repository and running workloads; secrets management fragmented across hydration templates.
- **Urgency:** **HIGH (Weeks 2–3 / P1)**
- **Recommended Action:** Bootstrap Flux CD v2 targeting `kubernetes/bootstrap/`, enforce Trunk-Based GitOps with protected `main` branch, and standardize all secrets on Mozilla SOPS + Age.

### Priority 5: Eliminate Operational Over-Engineering (Maintainability & Efficiency)
- **The Problem:** Project roadmaps contain proposals for enterprise tooling that exceed current scale and hardware capacity (e.g. self-hosted Keycloak consuming 1.2GB RAM, Flagger canary pipelines for 1-replica apps, GCP Cloud Run event buses).
- **Business Impact:** Operator cognitive burnout, wasted engineering hours, and cluster memory starvation.
- **Technical Impact:** Unnecessary software dependencies and fragile deployment pipelines.
- **Urgency:** **MEDIUM (Week 4 / P1)**
- **Recommended Action:** Practice aggressive technical parsimony: substitute Keycloak with Cloudflare Access (0MB RAM), defer Flagger in favor of native Kubernetes rolling updates, and execute document processing locally on the 1TB HDD.

---

## 11. Questions / Missing Information Required From User

To ensure the architectural baseline remains 100% aligned with your operational reality, please review the following high-value questions:

1. **OCI Tenancy State:** Do you currently possess an active, verified Oracle Cloud Infrastructure (OCI) account with Always Free tenancy access in `ap-mumbai-1`?  
   *(Recommended: If not yet provisioned, register immediately and create a ₹1 budget alarm to avoid credit card surprises).*
2. **Cloudflare Zone Ownership:** Is the root domain `vijaysingh.cloud` currently active with Cloudflare as its authoritative nameserver, allowing programmatic creation of DNS records and Tunnels via the Cloudflare Terraform provider?  
   *(Recommended: Yes, keep `vijaysingh.cloud` authoritative on Cloudflare to utilize free universal SSL, CDN caching, and Zero Trust Access).*
3. **Preiya Portfolio Custom Domain Intent:** Does your sister Preiya plan to operate exclusively under `preiya.vijaysingh.cloud` for the foreseeable future, or is a custom root domain (e.g. `coachpreiya.com`) planned within the next 6 months?  
   *(Recommended: Launch immediately on `preiya.vijaysingh.cloud` at ₹0 cost; Cloudflare Tunnels allow zero-downtime custom domain CNAME attachment whenever purchased).*
4. **Current Ingress VM Location:** Can you confirm whether the active GCP VM gateway is currently running in `us-east1` (South Carolina) as indicated in `current vs future.md` or `asia-south1` (Mumbai) as described in `process/01-existing-infrastructure-overview.md`?  
   *(Recommended: Regardless of current region, decommissioning GCP in favor of Cloudflare Tunnels solves both cost and cross-regional latency).*
5. **Ansible Vault Passphrase Storage:** Is the master Ansible Vault passphrase (`.ansible_vault_pass`) backed up in an external encrypted password manager outside the Mini PC?  
   *(Recommended: Ensure the vault password and newly generated Age private key are safely stored in an offline password manager like Bitwarden or 1Password).*
